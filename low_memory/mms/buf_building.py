...
"""
# OLD
def buld_mms_buffers(dnn, phases_per_layer, verbose=False):

    Build MMS buffers for a single DNN
    :param dnn: dnn
    ...
    :return: a set of CSDF buffers with MMS

    annotate_layers_with_phases(dnn, phases_per_layer)
    annotate_with_sim_time(dnn)

    csdf = dnn_to_csfd_one_to_one(dnn, fuse_self_loops=True)
    # consistency = check_csdfg_consistency(csdf, verbose=True)

    # build naive (non-dp, non-reuse) buffers
    csdf_buffers = build_naive_csdfg_buffers(csdf)
    # force_self_loops_reuse(reuse_csdf_buffers)
    sim_trace = simulate_execution_asap(csdf, csdf_buffers,
                                        max_samples=1,
                                        proc_num=1,
                                        trace_memory_access=True,
                                        verbose=False)
    sim_trace.sort_tasks_by_start_time()
    # minimize sizes of buffers (dp, non-reuse buffers)
    minimize_csdfg_buf_sizes(sim_trace, csdf_buffers)
    # reuse buffers (mms: dp + reuse buffers)
    reuse_csdf_buffers = build_csdfg_reuse_buffers_from_sim_trace(sim_trace, csdf_buffers)

    return reuse_csdf_buffers


def build_mms_buffers_multi_dnn(dnns, phases_per_layer_per_dnn, verbose=False):

    Build MMS buffers for multiple DNNs
    :param dnns: dnn
    :param phases_per_layer_per_dnn: dictionary where key = dnn name,
    value = dictionary where key = name of layer in the DNN, value=
    number of phases performed by every layer of a DNN
    :param verbose: print details
    :return: a set of CSDF buffers with MMS
    csdf_buffers_per_csdf = []

    for dnn in dnns:
        annotate_layers_with_phases(dnn, phases_per_layer_per_dnn[dnn.name])
        annotate_with_sim_time(dnn)
        ...
        # reuse buffers (mms: dp + reuse buffers)
        reuse_csdf_buffers = build_csdfg_reuse_buffers_from_sim_trace(sim_trace, csdf_buffers)
        csdf_buffers_per_csdf.append(reuse_csdf_buffers)

        reset_phases(dnn)

    # reuse (share) buffers among csdf
    shared_buffers = reuse_buffers_among_csdf(csdf_buffers_per_csdf)
